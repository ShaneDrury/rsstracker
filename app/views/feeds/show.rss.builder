...
xml.rss :version => '2.0',
  ...
  xml.channel do
    ...
    xml.language 'en'
    xml.itunes :image, href: url_for(@feed.thumbnail) if @feed.thumbnail.attached?
    xml.link root_url
    ...
    @feed.episodes.includes(:fetch_status).each do |episode|
      ...
      xml.item do
        ...
        xml.media :thumbnail, url: url_for(episode.thumbnail) if episode.thumbnail.attached?
        xml.media :content,
